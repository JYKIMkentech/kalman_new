...
%% 1. Parameters
n = 21;                    % RC 요소의 수 (DRT를 부드럽게 하기 위해 증가)
t = 0:0.01:100;            % 시간 벡터 (초)
dt = t(2) - t(1);          % 시간 간격
num_scenarios = 10;        % 전류 시나리오 수
lambda = 0.05;             % 정규화 파라미터
...
%% 5. True DRT Parameters (Gamma) - Lognormal 분포 사용
% 로그정규분포의 평균과 표준편차 설정
mean_tau = 10;      % tau의 평균
std_tau = 5;        % tau의 표준편차
...
Gamma_true = Gamma_true / max(Gamma_true);  % 최대값을 1로 정규화
...
Gamma_estimated_analytic = zeros(num_scenarios, n);   % 해석적 DRT 추정치 저장
V_est_all = zeros(num_scenarios, length(t));          % 모든 시나리오의 추정 전압 저장
V_sd_all = zeros(num_scenarios, length(t));           % 모든 시나리오의 합성 측정 전압 저장

%% 8. Voltage Synthesis and Plotting Setup
figure(1);  
sgtitle('각 시나리오별 전류 및 전압');
...
for s = 1:num_scenarios
    fprintf('시나리오 %d/%d 처리 중...\n', s, num_scenarios);
    
    % 시나리오의 전류 (m x 1)
    ik = ik_scenarios(s, :)';  % 열 벡터로 전환 (m x 1)
    
    %% 9.1. Initialize Voltage
    V_est = zeros(length(t),1);      % n-RC 모델을 통한 추정 전압 (m x 1)
    R0 = 0.1;                         % 오믹 저항 (Ohms)
    OCV = 3.7;                        % 개방 회로 전압 (V)
    V_RC = zeros(n, length(t));       % 각 요소의 RC 전압 (n x m)
    
    %% 9.2. Initial Voltage Calculation (First Time Step)
    for i = 1:n
        V_RC(i, 1) = ik(1) * Gamma_true(i) * (1 - exp(-dt / tau_discrete_log(i)));
    end
    ...
    for k_idx = 2:length(t)
        for i = 1:n
            % 이전 시간 단계의 데이터를 사용하여 RC 전압 계산
            V_RC(i, k_idx) = exp(-dt / tau_discrete_log(i)) * V_RC(i, k_idx-1) + ...
                              Gamma_true(i) * (1 - exp(-dt / tau_discrete_log(i))) * ik(k_idx);       
        end
    ...
    rng(s);  % 각 시나리오에 대해 노이즈 재현성 보장
    noise_level = 0.01;
    V_sd = V_est + noise_level * randn(size(V_est));  % V_sd = 합성 측정 전압 (m x 1)
    ...
    % 해석적 Gamma 계산
    Gamma_analytic = A_matrix \ b_vector;    % (n x 1)
    
    % 비음수 강제
    Gamma_analytic(Gamma_analytic < 0) = 0;
    
    % 해석적 DRT 저장
    Gamma_estimated_analytic(s, :) = Gamma_analytic';  % 행 벡터로 저장 (1 x n)
...
for s = 1:num_scenarios
    ...
    % True Gamma vs ln(tau) 플롯
    plot(theta_j, Gamma_true, 'k-', 'LineWidth', 2, 'DisplayName', 'True Gamma');
    
    % 해석적 Gamma vs ln(tau) 플롯
    plot(theta_j, Gamma_estimated_analytic(s, :), 'r--', 'LineWidth', 1.5, 'DisplayName', 'Estimated Gamma');
...
for s = 1:num_scenarios
    plot(theta_j, Gamma_estimated_analytic(s, :), '--', 'Color', colors(s,:), 'LineWidth', 1, 'DisplayName', ['Estimated Gamma S', num2str(s)]);
end
...
xlabel('ln(\tau) (로그 시간 상수)');
ylabel('\Gamma (감마)');
title('모든 시나리오에 대한 DRT 추정 비교 (해석적 솔루션)');
legend('Location', 'BestOutside');
grid on;
hold off;
...
disp(['Gamma_estimated_analytic: ', mat2str(size(Gamma_estimated_analytic))]);
