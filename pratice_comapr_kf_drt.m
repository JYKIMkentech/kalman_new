...
all_SOC_true = cell(num_trips, 1);
all_SOC_HPPC = cell(num_trips, 1);
all_SOC_DRT = cell(num_trips, 1);
all_Vt_meas = cell(num_trips, 1);
all_Vt_HPPC = cell(num_trips, 1);
all_Vt_DRT = cell(num_trips, 1);
all_time = cell(num_trips, 1);
all_current = cell(num_trips, 1);

% 이전 트립의 마지막 시간을 저장하기 위한 변수 초기화
previous_time_end = 0;

% 초기 상태 설정
% 첫 번째 트립의 초기 SOC 추정 (전압 기반)
initial_voltage = udds_data(1).V(1);
...
for trip_num = 1:num_trips
    ...
    % 시간 벡터 조정 (이전 트립의 마지막 시간과 이어지도록)
    trip_time = trip_time + previous_time_end;
    previous_time_end = trip_time(end);
    ...
    for k = 2:num_samples
        ...
        % 상태 예측
        X_pred_HPPC = A_k_HPPC * [SOC_est_HPPC; V1_est_HPPC] + B_k_HPPC * ik;
        ...
        % SOC 예측 (쿨롱 카운팅)
        SOC_pred_DRT = SOC_est_DRT + (dt_k / (Config.cap * 3600)) * ik;
        SOC_pred_DRT = max(0, min(1, SOC_pred_DRT));

        % 현재 SOC에 대한 gamma 값 보간
        gamma_current = interp1(soc_sorted, gamma_sorted, SOC_pred_DRT, 'linear', 'extrap')'; % 열 벡터로 변환

        % 각 RC 소자에 대한 R_i 및 C_i 계산
        delta_theta = theta_discrete(2) - theta_discrete(1);
        R_i = gamma_current .* delta_theta; % 요소별 곱셈
        C_i = tau_discrete ./ R_i;          % 요소별 나눗셈

        % RC 전압 예측
        exp_term = exp(-dt_k ./ (R_i .* C_i)); % 공통 부분 계산
        V_RC_pred = X_est_DRT(2:end) .* exp_term + (R_i .* (1 - exp_term)) .* ik; % 요소별 곱셈 사용

        % 상태 예측
        X_pred_DRT = [SOC_pred_DRT; V_RC_pred];

        % 공분산 예측
        A_DRT = diag([1; exp_term]);
...
for trip_num = 1:num_trips
    combined_time = [combined_time; all_time{trip_num}];
    combined_SOC_true = [combined_SOC_true; all_SOC_true{trip_num}];
    combined_SOC_HPPC = [combined_SOC_HPPC; all_SOC_HPPC{trip_num}];
    combined_SOC_DRT = [combined_SOC_DRT; all_SOC_DRT{trip_num}];
    combined_Vt_meas = [combined_Vt_meas; all_Vt_meas{trip_num}];
    combined_Vt_HPPC = [combined_Vt_HPPC; all_Vt_HPPC{trip_num}];
    combined_Vt_DRT = [combined_Vt_DRT; all_Vt_DRT{trip_num}];
    combined_current = [combined_current; all_current{trip_num}];
end

% Subplot 1: SOC 비교
subplot(3,1,1);
plot(combined_time, combined_SOC_true * 100, 'k-', 'LineWidth', 1, 'DisplayName', 'True SOC');
hold on;
plot(combined_time, combined_SOC_HPPC * 100, 'b--', 'LineWidth', 1, 'DisplayName', 'Estimated SOC (HPPC)');
plot(combined_time, combined_SOC_DRT * 100, 'r--', 'LineWidth', 1, 'DisplayName', 'Estimated SOC (DRT)');
xlabel('Time [s]');
ylabel('SOC [%]');
...
% Subplot 2: 터미널 전압 비교
subplot(3,1,2);
plot(combined_time, combined_Vt_meas, 'k-', 'LineWidth', 1, 'DisplayName', 'Measured Voltage');
hold on;
plot(combined_time, combined_Vt_HPPC, 'b--', 'LineWidth', 1, 'DisplayName', 'Estimated Voltage (HPPC)');
plot(combined_time, combined_Vt_DRT, 'r--', 'LineWidth', 1, 'DisplayName', 'Estimated Voltage (DRT)');
xlabel('Time [s]');
ylabel('Terminal Voltage [V]');
...
% Subplot 3: 전류 프로파일
subplot(3,1,3);
plot(combined_time, combined_current, 'g-', 'LineWidth', 1);
xlabel('Time [s]');
ylabel('Current [A]');
