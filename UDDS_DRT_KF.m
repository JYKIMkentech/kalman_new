...
for k = 2:N
    ...
    gamma_k = interp1(soc_sorted, gamma_sorted, SOC_est(k-1), 'linear', 'extrap');  % 1 x n_RC
    R0_k = interp1(soc_sorted, R0_est_all, SOC_est(k-1), 'linear', 'extrap');      % 스칼라
    
    % 시간 간격에 따라 A_k, B_k 매트릭스 업데이트
    A_k = diag(exp(-dt(k) ./ tau_discrete));  % n_RC x n_RC
    B_k = gamma_k' .* (1 - exp(-dt(k) ./ tau_discrete));  % n_RC x1
