...
%% 이전 1RC 모델의 R1과 C1 값 불러오기
% optimized_params_struct_final.mat 파일 로드
load('optimized_params_struct_final_ver2.mat'); % 파일 경로를 실제 위치로 수정하세요

% 로드된 데이터에서 R1과 C1 값을 추출합니다.
% 이 예시에서는 optimized_params_struct라는 구조체에 R1, C1이 있다고 가정합니다.
% 각 스텝에 해당하는 R1과 C1 값을 배열로 저장합니다.
R1_values = [optimized_params_struct_final_ver2.R1]; % 스텝 순서대로 R1 값 배열
C1_values = [optimized_params_struct_final_ver2.C]; % 스텝 순서대로 C1 값 배열
...
optimized_params = table('Size', [0 6], ...
    'VariableTypes', {'double', 'double', 'double', 'double', 'double', 'double'}, ...
    'VariableNames', {'R0', 'R1', 'C1', 'R2', 'C2', 'SOC'});
...
for idx = 1:length(step_dis)
    ...
    % R0 설정: 첫 번째 R 값을 사용하거나 필요에 따라 수정
    if length(data(i_step).R) >= 1
        R0 = data(i_step).R(1); % 첫 번째 R 값을 R0로 저장
    else
        R0 = 0;
    end
    
    % 이전 1RC 모델에서 R1과 C1 값 가져오기
    % 인덱싱이 정확한지 확인하세요
    R1 = R1_values(idx);
    C1 = C1_values(idx);
    
    % 초기 파라미터 설정 [R2, C2]
    initial_params = [0.01, 3000];
    
    % 코스트 함수 정의 (RMSE 계산)
    cost_function = @(params) V - ECM_2RC(params, OCV, I, dt, R0, R1, C1);
    ...
    % 피팅 수행
    [fitted_params, resnorm, residual] = lsqnonlin(cost_function, ...
        initial_params, lb, ub, options);
    
    % 모델 전압 계산
    V_model = ECM_2RC(fitted_params, OCV, I, dt, R0, R1, C1);
    ...
    % 최적화된 파라미터를 테이블에 추가
    new_row = {R0, R1, C1, fitted_params(1), fitted_params(2), data(i_step).SOC};
    optimized_params = [optimized_params; new_row];
end

% 열 이름 설정
optimized_params.Properties.VariableNames = {'R0', 'R1', 'C1', 'R2', 'C2', 'SOC'};
...
function V_model = ECM_2RC(params, OCV, I, dt, R0, R1, C1)
    % 파라미터 분리
    R2 = params(1);
    C2 = params(2); 

    % 고정된 R1과 C1을 사용하여 tau1 계산
    tau1 = R1 * C1;
    % 피팅할 R2와 C2를 사용하여 tau2 계산
    tau2 = R2 * C2;
