...
% 프로세스 잡음 공분산 (HPPC 기반)
% Q_HPPC = zeros(2);
% Q_HPPC(1,1) = 4e-14; % SOC에 대한 프로세스 잡음 분산
% Q_HPPC(2,2) = (0.04)^2; % 각 V_RC_i에 대한 프로세스 잡음 분산
...
% 측정 잡음 공분산 (HPPC 기반)
R_HPPC = 5.25e-16; % 측정 잡음 특성에 따라 조정
...
% Subplot 3: 전류 프로파일
subplot(3,1,3);
plot(all_time_concat, all_current_concat, 'g-', 'LineWidth', 1.0, 'DisplayName', 'Current Profile');
xlabel('Time [s]');
ylabel('Current [A]');
