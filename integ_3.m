...
% 비용 함수 정의 (시간 가중치 포함)
cost_function = @(params) func_cost(params, fixed_R0, initial_soc, udds_current, udds_voltage, Config, soc_values, ocv_values, udds_time);
...
function cost = func_cost(params, R0, initial_soc, current, voltage, Config, soc_values, ocv_values, time)
    ...
    % 시간 가중치를 적용한 잔차 계산
    weighted_residuals = time_weights .* residuals;
