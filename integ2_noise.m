...
voltage_noise_std = 0.02; % Standard deviation of voltage noise
noisy_current = udds_current + current_noise_std * randn(size(udds_current));
noisy_voltage = udds_voltage + voltage_noise_std * randn(size(udds_voltage));
...
% Initialize noisy true SOC using coulomb counting
noisy_true_SOC = zeros(length(udds_current), 1);
noisy_true_SOC(1) = initial_soc;
...
for k = 2:length(udds_current)
    ...
    noisy_true_SOC(k) = noisy_true_SOC(k-1) + (noisy_current(k) * delta_t) / (Config.cap * 3600); % 실제 soc = 전 soc + i * dt/q_total (sampling 시간 동안 흐르는 전류)
    ...
    [SOC_est(k), V1_est(k), Vt_est(k), P] = soc_estimation(SOC_est(k-1), V1_est(k-1), noisy_voltage(k), noisy_current(k), Config, P, unique_soc_values, unique_ocv_values); % 추정 코드
end

% Create the result matrix as a Nx5 array
result_matrix = [true_SOC, noisy_true_SOC, SOC_est, R0_used, R1_used, C_used];

% Define column names
column_names = {'True_SOC', 'Noisy_True_SOC', 'Estimated_SOC', 'R0', 'R1', 'C'};
...
plot(udds_time, noisy_true_SOC, 'g', 'LineWidth', 1.5);
plot(udds_time, SOC_est, 'r--', 'LineWidth', 1.5);
xlabel('Time (s)');
ylabel('SOC');
title('True SOC vs Noisy True SOC vs Estimated SOC during UDDS Cycle');
legend('True SOC', 'Noisy-True SOC', 'Noisy-Estimated SOC');
grid on;
...
save('result_matrix.mat', 'udds_time', 'true_SOC', 'noisy_true_SOC', 'SOC_est', 'udds_voltage', 'Vt_est');
