...
% 전류와 전압 그래프 그리기
plot(Time_combined, Current_combined, 'Color', colors(1,:), 'LineWidth', 1.5);
hold on;
yyaxis right
plot(Time_combined, Voltage_combined, 'Color', colors(2,:), 'LineWidth', 1.5);
...
if ~isempty(step64_index_in_step_dis)
    ...
    % 모델 전압 그래프 추가 (검은색 점선)
    plot(data64_t, voltage_model_step64_adjusted, '--', 'Color', colors(3,:), 'LineWidth', 1.5);
end

xlabel('Time (s)');
ylabel('Current (A) / Voltage (V)');
legend('Current', 'Voltage', 'Model Voltage');
title('Time vs Current and Voltage');
grid on;
