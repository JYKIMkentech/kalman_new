...
for i = 1:length(step_dis)
    figure(1); hold on; % Use a separate figure for plotting
...
% 구조체 생성
optimized_params_struct_final_ver2 = struct('R0', [], 'R1', [], 'C', [], 'SOC', [], 'Crate', [], 'm', []); % 'Crate' 필드 수정
...
for i = 1:length(step_dis)
    ...
    % m 후보군 정의
    m_candidates = [2 / data(step_dis(i)).timeAt632, 1 / data(step_dis(i)).timeAt632, 1.05, 1.4, 0.5 / data(step_dis(i)).timeAt632];
    ...
    for m_idx = 1:num_m
        ...
        if step_duration > 0 
            ...
            problem = createOptimProblem('fmincon', 'objective', @(params) cost_function_discrete(params, time_exp, deltaV_exp, data(step_dis(i)).I, m, data(step_dis(i)).R0, deltaV_exp(1)), ...
    ...
    if ~isnan(best_m)
        optimized_params_struct_final_ver2(i).R0 = data(step_dis(i)).R0; % R0 고정된 값 사용
        optimized_params_struct_final_ver2(i).R1 = best_params(1);
        optimized_params_struct_final_ver2(i).C = best_params(2);
        optimized_params_struct_final_ver2(i).SOC = mean(data(step_dis(i)).SOC); % 평균 SOC 값을 저장
        optimized_params_struct_final_ver2(i).Crate = avgI / data(step_dis(2)).avgI; % 평균 전류 저장
        optimized_params_struct_final_ver2(i).m = best_m; % 선택된 m 값 저장
    else
        optimized_params_struct_final_ver2(i).R0 = NaN;
        optimized_params_struct_final_ver2(i).R1 = NaN;
        optimized_params_struct_final_ver2(i).C = NaN;
        optimized_params_struct_final_ver2(i).SOC = NaN;
        optimized_params_struct_final_ver2(i).Crate = NaN;
        optimized_params_struct_final_ver2(i).m = NaN;
    end
end
...
for i = 1:length(step_dis)
    ...
    if (data(step_dis(i)).t(end) - data(step_dis(i)).t(1)) > 0
        ...
        deltaV_exp = data(step_dis(i)).deltaV;
        time_exp = data(step_dis(i)).t;
        I_exp = data(step_dis(i)).I;
        avgI = data(step_dis(i)).avgI;
        optimized_R0 = optimized_params_struct_final_ver2(i).R0;
        optimized_R1 = optimized_params_struct_final_ver2(i).R1;
        optimized_C = optimized_params_struct_final_ver2(i).C;
        m = optimized_params_struct_final_ver2(i).m;
        timeAt632 = data(step_dis(i)).timeAt632;
        deltaV_init = deltaV_exp(1); % Initial voltage difference for the model
        
        % Generate model voltage using the updated discrete model
        voltage_model = model_func_discrete_custom(time_exp, optimized_R0, optimized_R1, optimized_C, I_exp, deltaV_init);
        ...
        soc_text = sprintf('SOC: %.2f%%', optimized_params_struct_final_ver2(i).SOC * 100);
        crate_text = sprintf('C-rate: %.2f', optimized_params_struct_final_ver2(i).Crate);
        m_text = sprintf('m: %.2f', optimized_params_struct_final_ver2(i).m);
...
% Extract SOC, C-rate, R0, R1, C from optimized_params_struct_final_ver2
SOC_vals = [optimized_params_struct_final_ver2.SOC]';
Crate_vals = [optimized_params_struct_final_ver2.Crate]';
R0_vals = [optimized_params_struct_final_ver2.R0]';
R1_vals = [optimized_params_struct_final_ver2.R1]';
C_vals = [optimized_params_struct_final_ver2.C]';

% Remove entries with NaN values
valid_idx = ~isnan(SOC_vals) & ~isnan(Crate_vals) & ~isnan(R0_vals) & ~isnan(R1_vals) & ~isnan(C_vals);
SOC_vals = SOC_vals(valid_idx);
Crate_vals = Crate_vals(valid_idx);
R0_vals = R0_vals(valid_idx);
R1_vals = R1_vals(valid_idx);
C_vals = C_vals(valid_idx);

% Define grid for interpolation
num_grid = 100; % Number of grid points in each dimension
SOC_grid = linspace(min(SOC_vals), max(SOC_vals), num_grid);
Crate_grid = linspace(min(Crate_vals), max(Crate_vals), num_grid);
[SG, CG] = meshgrid(SOC_grid, Crate_grid);

% Interpolate R0, R1, C over the grid
R0_grid = griddata(SOC_vals, Crate_vals, R0_vals, SG, CG, 'cubic');
R1_grid = griddata(SOC_vals, Crate_vals, R1_vals, SG, CG, 'cubic');
C_grid = griddata(SOC_vals, Crate_vals, C_vals, SG, CG, 'cubic');
...
% Updated cost function to use the custom discrete model with deltaV_init
function cost = cost_function_discrete(params, time, deltaV, I, m, R0, deltaV_init)
    R1 = params(1);
    C = params(2);
    
    % 모델 함수를 사용하여 예측 전압 계산 (Custom Discrete Time with deltaV_init)
    voltage_model = model_func_discrete_custom(time, R0, R1, C, I, deltaV_init);
...
% Custom discrete-time model function based on the user-provided equation
function voltage = model_func_discrete_custom(time, R0, R1, C, I, deltaV_init)
    % Initialize voltage array
    voltage = zeros(size(time));
    
    % Initial voltage condition based on deltaV_exp
    voltage(1) = deltaV_init; % Set to first deltaV_exp value
    
    % Iterate over each time step to compute voltage
    for k = 2:length(time)
        dt = time(k) - time(k-1); % Time step
     
        alpha = exp(-dt / (R1 * C));
        % 수정된 부분: I를 벡터 전체가 아닌 현재 시점 k의 값으로 변경
        voltage(k) =  I(k) * R0 + voltage(k-1) * alpha + I(k) * R1 * (1 - alpha);
    end
end

% Define a function to create 3D Surface and Contour Plots
