...
for idx = 1:length(step_dis)
    ...
    % 초기 파라미터 설정 [R0, R1, R2, C1, C2]
    initial_params = [0.01, 0.01, 0.01, 1000, 3000];
    
    % fmincon용 비용 함수 정의 (잔차의 제곱합)
    cost_function = @(params) sum((V - ECM_2RC(params, OCV, I, dt)).^2);
    ...
    options = optimoptions('fmincon', ...
        'Display', 'off', ...
        'Algorithm', 'interior-point', ... % 다른 알고리즘도 선택 가능
        'MaxIterations', 1000, ...
    ...
    % fmincon을 사용하여 피팅 수행
    [fitted_params, fval, exitflag, output] = fmincon(cost_function, initial_params, [], [], [], [], lb, ub, [], options);
    
    % 모델 전압 계산
    V_model = ECM_2RC(fitted_params, OCV, I, dt);
    ...
    % 최적화된 파라미터를 테이블에 추가
    new_row = {fitted_params(1), fitted_params(2), fitted_params(3), ...
               fitted_params(4), fitted_params(5), data(i_step).Crate, data(i_step).SOC};
...
function V_model = ECM_2RC(params, OCV, I, dt)
    % 파라미터 분리
    R0 = params(1);
    R1 = params(2);
    R2 = params(3);
