...
%% 데이터 로드
% 1RC 모델 결과 로드
% 'optimized_params_struct_final_ver2.mat' 파일에는 각 스텝별 R0, R1, C1이 저장되어 있다고 가정합니다.
...
%% deltaV 계산 (전압 변화량)
% 수정된 부분: deltaV를 스텝 내 초기 전압에서 현재 전압으로 계산
...
for idx = 1:length(step_dis)
    ...
    R0 = optimized_params_struct_final_ver2(idx).R0;
    R1 = R1_values(idx);
    C1 = C1_values(idx);

    % R0, R1, C1이 유효한지 확인
    if isnan(R0) || isnan(R1) || isnan(C1)
        optimized_params_struct_final_ver2_2RC(idx).R0 = R0;
        optimized_params_struct_final_ver2_2RC(idx).R1 = R1;
        optimized_params_struct_final_ver2_2RC(idx).C1 = C1;
        optimized_params_struct_final_ver2_2RC(idx).R2 = NaN;
        optimized_params_struct_final_ver2_2RC(idx).C2 = NaN;
        optimized_params_struct_final_ver2_2RC(idx).SOC = data(i).SOC;
        optimized_params_struct_final_ver2_2RC(idx).Crate = avgI / mean([data(step_dis).avgI]);
        continue; % 다음 스텝으로 건너뜁니다.
    end
...
for idx = 1:length(step_dis)
    ...
    if (data(i).t(end) - data(i).t(1)) >= 0
        ...
        soc = data(i).SOC; % SOC를 data 구조체에서 직접 가져옴
        crate = optimized_params_struct_final_ver2_2RC(idx).Crate;

        % Check for valid parameters
        if any(isnan([optimized_R0, optimized_R1, optimized_R2, optimized_C1, optimized_C2]))
            warning('Step %d has NaN parameters. Skipping plot.', idx);
            hold off;
...
% Extract SOC, C-rate, R0, R1, R2, C1, C2 from optimized_params_struct_final_ver2_2RC
...
valid_idx = ~isnan(SOC) & ~isnan(Crate) & ~isnan(R0) & ~isnan(R1) & ~isnan(R2) & ~isnan(C1) & ~isnan(C2);
SOC = SOC(valid_idx);
...
% Interpolate R0, R1, R2, C1, C2 over the grid
...
% saveas(gcf, 'R2_plot.png');
% saveas(gcf, 'C1_plot.png');
