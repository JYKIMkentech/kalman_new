...
all_R0 = cell(num_trips-1, 1);
all_R1 = cell(num_trips-1, 1);
all_C1 = cell(num_trips-1, 1);
...
try
    for trip_num = 1:num_trips-1
       ...
       %% 7.3. 전압에 1% Gaussian Random Noise 추가
        ...
        % 추가: R₀, R₁, C 값을 저장할 배열 초기화
        R0_save = zeros(num_samples, 1);
        R1_save = zeros(num_samples, 1);
        C1_save = zeros(num_samples, 1);
...
%             C1_interp = max(C1_interp, 1e-5);
        ...
        % R₀, R₁, C₁ 값을 셀 배열에 저장
        all_R0{trip_num} = R0_save;
        all_R1{trip_num} = R1_save;
        all_C1{trip_num} = C1_save;
