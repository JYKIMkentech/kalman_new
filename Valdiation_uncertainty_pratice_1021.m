    ...
    for k_idx = 1:length(t)
        for i = 1:n
            if k_idx == 1
                ...
                W(k_idx, i) = W(k_idx-1, i) * exp(-dt / tau_discrete(i)) + ik(k_idx) * (1 - exp(-dt / tau_discrete(i))) * delta_theta;
            end
        end
    end
...
gamma_fold_estimates = cell(length(candidate_lambdas), length(validation_folds)); % Store gamma estimates for each fold
...
for l = 1:length(candidate_lambdas)
    ...
    for fold = 1:length(validation_folds)
        ...
        sum_WtV = zeros(n, 1);   % Initialize sum(W_s^T * y_s)
        ...
        %% Analytical Solution for gamma_train
        gamma_train = (sum_WtW + regularization_term) \ sum_WtV;
        gamma_train(gamma_train < 0) = 0; % Enforce non-negativity
        
        %% Store gamma estimates for uncertainty calculation
        gamma_fold_estimates{l, fold} = gamma_train;
        ...
        for s_val = val_set
            ...
            V_pred = W_val * gamma_train + R0 * ik_val + OCV;
...
%% Extract gamma estimates at optimal lambda for uncertainty estimation
gamma_estimates_optimal = cell2mat(gamma_fold_estimates(optimal_idx, :)'); % n x 폴드 수

% Calculate mean and standard deviation of gamma estimates across folds (dimension 2)
gamma_mean = mean(gamma_estimates_optimal, 2); % n x 1
gamma_std = std(gamma_estimates_optimal, 0, 2); % n x 1

% Ensure theta_discrete is a column vector
theta_discrete = theta_discrete(:); % n x 1

%% Verify sizes before plotting
disp('Size of theta_discrete:');
disp(size(theta_discrete));

disp('Size of gamma_mean:');
disp(size(gamma_mean));

disp('Size of gamma_std:');
disp(size(gamma_std));

%% Plot gamma mean and uncertainty using errorbar
figure;
errorbar(theta_discrete, gamma_mean, gamma_std, 'r--', 'LineWidth', 2, 'DisplayName', 'Optimized \gamma(\theta) \pm 1 STD');
hold on;
plot(theta_discrete, gamma_discrete_true, 'k-', 'LineWidth', 2, 'DisplayName', 'True \gamma(\theta)');
xlabel('ln(\tau) = \theta');
ylabel('\gamma(\theta)');
title('Gamma Mean and Uncertainty (\pm1 STD) vs True Gamma');
...
sum_WtV_final = zeros(n, 1);   % Initialize sum(W_s^T * y_s)
...
% Compute gamma_final using optimal lambda
gamma_final = (sum_WtW_final + regularization_term) \ sum_WtV_final;
gamma_final(gamma_final < 0) = 0; % Enforce non-negativity
...
for s_test = test_set
    ...
    % Predict V_pred_test
    V_pred_test = W_test * gamma_final + R0 * ik_test + OCV;
...
%% Plot Final DRT Comparison: True vs Optimized Gamma
figure;
plot(theta_discrete, gamma_discrete_true, 'k-', 'LineWidth', 2, 'DisplayName', 'True \gamma(\theta)');
hold on;
...
xlabel('ln(\tau) = \theta');
ylabel('\gamma(\theta)');
title('Final Comparison of True DRT and Optimized DRT');
legend('Location', 'Best');
grid on;
hold off;
